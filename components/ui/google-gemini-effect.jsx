...
export const GoogleGeminiEffect = ({ pathLengths, className }) => {
  ...
  // Independent offsets with hardcoded random-like x values
  ...
  // Calculate start and end points along each path
  useEffect(() => {
    ...
    const newPoints = pathRefs.current.map((path) => {
      ...
      // Only start (0%) and end (100%) points
      return [0, length].map((pos) => path.getPointAtLength(pos));
    });
  ...
  return (
    <div className={cn("sticky top-0", className)}>
      ...
      {/* Logos and TRIG texts with black rounded squares */}
      {points.map((linePoints, i) =>
        linePoints.map((point, j) => {
          const index = i * 2 + j;
          const offset = elementOffsets[index] || { x: 0, y: 0 };

          if (j === 0) {
            return (
              <div
                key={`logo-${i}`}
                className="absolute z-50 flex items-center justify-center bg-black rounded-lg w-12 h-12"
                style={{
